...
describe("TemplateExpression", () => {
	...
	describe("YAML synthesis", () => {
		it("should generate correct YAML for simple condition", () => {
			const expr = If(new Eq(variables.get("runTests"), "true"), { script: "npm test" });
			const yaml = expr.synthesize();
		...
		it("should generate correct YAML for complex condition", () => {
			...
			expect(yaml).toEqual({
				"${{ if and(eq(variables.env, 'production'), ne(variables.skipDeployment, 'true')) }}": {
					script: "deploy.sh"
				}
			});
		});

		it("should handle array content in YAML", () => {
			const expr = If(new Eq(variables.get("runTests"), "true"), [{ script: "npm test" }, { script: "npm run coverage" }]);
			const yaml = expr.synthesize();

			expect(yaml).toEqual({
				"${{ if eq(variables.runTests, 'true') }}": [{ script: "npm test" }, { script: "npm run coverage" }]
			});
		});
	});
});
