...
const exampleJob = new Job({
	...
	steps: [
		...
		If(
			new Eq(variables.get("isMain"), true).and(new Eq(variables.get("Build.Reason"), "PullRequest").not()),
			new TaskStep({
				task: "PowerShell@2",
		...
		If(
			new Eq(variables.get("Agent.OS"), "Windows_NT"),
			new TaskStep({
				task: "PowerShell@2",
		...
		If(
			new Eq(variables.get("Agent.OS"), "Linux"),
			new TaskStep({
				task: "Bash@3",
